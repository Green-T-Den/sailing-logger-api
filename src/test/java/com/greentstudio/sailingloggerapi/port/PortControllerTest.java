...
public class PortControllerTest {
  ...
  @MockBean private PortRepository portRepository;
  ...
  public void getShouldFetchAHalDocument() throws Exception {
    ...
    given(portRepository.findAll())
        .willReturn(
            Arrays.asList(new Port(1L, "testPort", boats1), new Port(2L, "testPort2", boats1)));

    mockMvc
        .perform(get("/ports").accept(MediaTypes.HAL_JSON_VALUE))
        .andDo(print())
        ...
        .andExpect(
            jsonPath("$._embedded.ports[0]._links.self.href", is("http://localhost/ports/1")))
        .andExpect(jsonPath("$._embedded.ports[0]._links.ports.href", is("http://localhost/ports")))
        .andExpect(
            jsonPath(
                "$._embedded.ports[0]._links.boats.href", is("http://localhost/ports/1/boats")))
        .andExpect(jsonPath("$._embedded.ports[1].id", is(2)))
        .andExpect(jsonPath("$._embedded.ports[1].strName", is("testPort2")))
        .andExpect(
            jsonPath("$._embedded.ports[1]._links.self.href", is("http://localhost/ports/2")))
        .andExpect(jsonPath("$._embedded.ports[1]._links.ports.href", is("http://localhost/ports")))
        .andExpect(
            jsonPath(
                "$._embedded.ports[1]._links.boats.href", is("http://localhost/ports/2/boats")))
        .andReturn();
  }
}
