...
@Component
public class PortRepresentationModelAssembler
    extends SimpleIdentifiableRepresentationModelAssembler<Port> {
  PortRepresentationModelAssembler() {
    super(PortController.class);
  }

  /**
   * Retain default links provided by {@link SimpleIdentifiableRepresentationModelAssembler}, but
   * add extra ones to each {@link Port}.
   *
    ...
    Objects.requireNonNull(resource.getContent())
        ...
        .ifPresent(
            id ->
                ...
                resource.add(
                    linkTo(methodOn(BoatController.class).findPorts(id)).withRel("boats")));
  }

  /**
   * Retain default links for the entire collection, but add extra custom links for the {@link Port}
   * collection.
   *
